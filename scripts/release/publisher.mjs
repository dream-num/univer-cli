...
import { assertReleaseContext, validateReleaseManifest } from "./policy.mjs";
...
export async function publishPreparedRelease(manifestPath, env = process.env) {
  ...
  const artifactRoot = dirname(absoluteManifestPath);
  const tarballPath = join(artifactRoot, manifest.tarball);
  ...
  const existingIntegrity = readRegistryIntegrity(manifest, artifactRoot, env);
  ...
  if (distTags?.[manifest.npmTag] !== manifest.version) {
    throw new Error(`Registry dist-tag ${manifest.npmTag} does not point to ${manifest.version}.`);
  }
  return manifest;
}
...
function readRegistryIntegrity(manifest, cwd, env) {
  ...
  if (/\bE404\b|404 Not Found/u.test(diagnostic)) return undefined;
  throw new Error(
