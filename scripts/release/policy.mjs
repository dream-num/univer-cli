...
export function validateReleaseManifest(manifest) {
  ...
  for (const field of ["entryCount", "packedSize", "unpackedSize"]) {
    if (
      !Number.isSafeInteger(manifest.packageMetrics?.[field]) ||
      manifest.packageMetrics[field] < 1
    ) {
      throw new Error(`Release manifest packageMetrics.${field} must be a positive integer.`);
    }
  }
  for (const field of ["required", "conditional"]) {
    if (
      !Array.isArray(manifest.dependencies?.[field]) ||
      manifest.dependencies[field].some((name) => typeof name !== "string" || name.length === 0)
    ) {
      throw new Error(`Release manifest dependencies.${field} must be a package-name array.`);
    }
  }
  return manifest;
}
