...
export function parseSdkUpdateVersion(argv) {
  ...
  if (version === undefined || !EXACT_SEMVER_PATTERN.test(version)) {
    throw new Error("SDK update requires --sdk_version <exact-semver> as its only argument.");
  }
  return version;
}

export function alignManifestSdkDependencies(manifest, version, workspaceNames = new Set()) {
