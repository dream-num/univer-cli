...
import { EMBED_CHILD_CREATE_OPTIONS } from "@univerjs-pro/embed";
...
export class UnitRegistry {
  ...
  async load(input: {
    ...
    try {
      ...
      // Source/Embed child 属于后台依赖，必须沿用 SDK 的 child 创建合同，避免它们
      // 成为 current Unit、触发主 Workbench 渲染并与 Host 挂载竞争。
      for (const reference of references) {
        instanceService.createUnit(
      ...
      for (const embedded of embeddedUnits) {
        if (createdUnitIds.includes(embedded.unitId)) {
          continue;
        }
        instanceService.createUnit(
          INSTANCE_TYPE[embedded.unitType],
          embedded.unitData as object,
          EMBED_CHILD_CREATE_OPTIONS,
        );
