...
import { createViewBaseResourceRefDataProviderRegistration } from "../src/view-base-resource-ref-data-provider.js";
  ...
  it("resolves a Base table by its canonical formula name", async () => {
    ...
    const registration = createViewBaseResourceRefDataProviderRegistration(() => ({
      ...
      univerInstanceService: {
        getUnit: () => ({ getSnapshot: () => snapshot }) as BaseDataModel
      }
    }));
    const ref: ResourceRefInput = {
      file: { kind: "self" },
