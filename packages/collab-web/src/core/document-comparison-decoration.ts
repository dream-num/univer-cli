...
import type { DocumentComparisonParagraph, DocumentComparisonRow, UnitStructuralDiffItem } from "@univer/unit-compare";
import type { UnitComparisonContextChange, UnitComparisonProductContext } from "@univer/collab-gateway-contract";
...
function decorateBody(
  ...
  const rows: DocumentComparisonRow[] = comparison.alignment
    ...
    .map((row) => ({
      id: row.id, stableId: row.stableId, kind: row.kind, moved: row.moved,
...
function buildTextRanges(
  segments: NonNullable<UnitComparisonContextChange["segments"]>,
  leftStart: number,
  rightStart: number
