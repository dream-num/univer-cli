...
} from "./comparison-focus";

type ComparisonUnitData =
  | IWorkbookData
  | IDocumentData
  | ISlideData
  | IBaseSnapshot
...
export async function createComparisonPane(
  ...
  let comparisonHighlights: ReturnType<
    typeof createNativeComparisonHighlightController
  > | undefined;
  ...
  try {
    ...
    comparisonHighlights = createNativeComparisonHighlightController({
      ...
      side: options.side,
      items: options.items,
      ...(options.selectedItemId === undefined
        ? {}
