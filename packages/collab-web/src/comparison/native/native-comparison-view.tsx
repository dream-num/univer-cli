...
import {
  ComparisonPageTabs,
  type ComparisonPageTabOption,
} from "../shared/scope-tabs";
...
import {
  ...
  type ComparisonPaneHandle,
} from "./comparison-pane";
  ...
  if (renderError !== null) {
    return (
      <div className="grid min-h-0 flex-1 place-items-center p-8 text-sm text-destructive">
        {t().diff.renderFailed}: {renderError}
      </div>
    );
  }
        ...
        <div className="grid min-h-0 grid-rows-[minmax(0,1fr)] bg-card">
          ...
          >
            <NativeDiffSide
              ...
              label={comparisonSideLabel(comparison.left)}
            ...
            <NativeDiffSide
              ...
              hideSlideAddControl={unitType === UNIT_TYPE_SLIDE}
              itemCount={scopedItems.length}
              label={comparisonSideLabel(comparison.right)}
            ...
            >
              ...
              <div className="truncate opacity-70">
                ...
                {item.changes.length > 0
                  ? ` · ${t().diff.changeCount(item.changes.length)}`
                  : ""}
              </div>
            </button>
          ))}
      ...
      <header className="flex min-w-0 items-center justify-between gap-3 border-b border-border bg-card px-4">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-[10px] font-semibold tabular-nums text-muted-foreground">
            {t().diff.changeCount(input.itemCount)}
          </span>
        </div>
      </header>
      {input.pageTabs.length > 0 ? (
        <ComparisonPageTabs
          ...
          ariaLabel={`${t().diff.side[input.side]} · ${t().diff.changedSlides}`}
