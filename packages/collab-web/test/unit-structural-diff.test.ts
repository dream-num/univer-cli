...
function baseChange(entityType: string, stableId: string, kind: "insert" | "delete" | "update", tableId = "t1"): UnitStructuralDiffItem {
  return {
    id: `${entityType}:${stableId}`, stableId, entityType, category: entityType,
    parentStableId: tableId, path: [entityType, tableId, stableId], label: stableId, kind,
    moved: false, changes: [], position: { left: null, right: null }, values: {},
  };
}

describe("stable-ID Unit structural diff", () => {
  it("renders Base dates, configured currency, and structured links as readable values", () => {
    expect(formatBaseCellValue(44927, { type: "date", config: { pattern: "yyyy-mm-dd" } })).toBe("2023-01-01");
    expect(formatBaseCellValue(1234.5, { type: "currency", config: { currencySymbol: "€", decimalPlaces: 2, separatorStyle: "periodComma" } })).toBe("€1.234,50");
    expect(formatBaseCellValue({ text: "Runbook", url: "https://example.com/runbook" }, { type: "link" })).toBe("Runbook");
  ...
  it("keeps renamed Base field headers specific to each comparison side", () => {
    const field = { id: "f", left: { name: "Planned budget" }, right: { name: "Approved budget" }, label: "Approved budget", status: "update" as const };
  ...
  it("aligns Base raw fields and records by stable ID and ignores view-only changes", () => {
    ...
    const [viewChange] = buildBaseTableDiff(left, viewOnlyRight, [baseChange("view", "grid", "update")]);
  ...
  it("uses one explicit Base grid geometry for both panes and hides internal record IDs", () => {
    ...
    const [diff] = buildBaseTableDiff(snapshot(120), snapshot(236), [baseChange("cell", "r1:title", "update", "tasks")]);
