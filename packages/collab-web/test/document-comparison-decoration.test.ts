...
function decorateDocumentComparisonSide(
  ...
  if (result.productContext?.type !== UniverInstanceType.UNIVER_DOC) throw new Error("Expected Doc alignment");
  ...
  return renderDocumentComparisonSide(current, peer, side, {
    ...
    alignment: result.productContext.paragraphAlignment.map((row, index) => {
      const parent = row.segmentPath === undefined ? undefined : `${row.segmentPath[0]}:${row.segmentPath[1]}`;
      const item = items.find((candidate) => candidate.entityType === "paragraph" &&
        candidate.stableId === row.stableId && candidate.parentStableId === parent);
      return {
        ...row, id: String(index), leftIndex: row.leftPosition, rightIndex: row.rightPosition,
...
describe("document comparison decoration", () => {
  it("keeps paragraph replacements inside their existing table cell when native paragraph IDs are regenerated", () => {
    const before = structuredDocument(["Intro", "table", "After"], { table: [["Label", "Old value", "Status"]] });
    const after = structuredDocument(["Intro", "table", "After"], { table: [["Label", "New value", "Status"]] });
  ...
  it.each([0, 1])("preserves grid column widths when inserting after a merged cell (gridBefore=%s)", (gridBefore) => {
    const before = structuredDocument(["Intro", "table", "After"], { table: [["a", "c"]] });
    const after = structuredDocument(["Intro", "table", "After"], { table: [["a", "Added content", "c"]] });
    for (const [data, widths] of [
      ...
      table.tableColumns = widths.map((v) => ({ size: { type: TableSizeType.SPECIFIED, width: { v } } }));
    ...
    for (const side of ["left", "right"] as const) {
      ...
      const actual = decorateDocumentComparisonSide(after, before, side === "left" ? "right" : "left");
      expect(ghost.body?.dataStream).toBe(actual.body?.dataStream);
      expect(validateDocumentStructure(ghost)).toEqual([]);
      expect(ghost.tableSource!["table-1"]!.tableColumns).toEqual(actual.tableSource!["table-1"]!.tableColumns);
      expect(ghost.tableSource!["table-1"]!.size).toEqual(actual.tableSource!["table-1"]!.size);
      expect(ghost.tableSource!["table-1"]!.tableRows[0]!.tableCells.map((cell) => cell.columnSpan ?? 1)).toEqual([2, 1, 1]);
    }
    expect(JSON.stringify([before, after])).toBe(original);
  });

  it.each(["row", "cell", "column"] as const)("keeps independent bilateral %s insertions in the same SDK alignment order", (kind) => {
    const create = (id: string): IDocumentData => kind === "column"
      ? structuredDocument(["Intro", "columns", "After"], { columns: ["a", id, "c"] })
      : structuredDocument(["Intro", "table", "After"], { table: kind === "row" ? [["a"], [id], ["c"]] : [["a", id, "c"]] });
    ...
    expect(coloredText(leftView)).toContainEqual({ text: "Left addition", color: "rgba(22, 163, 74, 0.24)", strike: false });
    expect(coloredText(leftView)).toContainEqual({ text: "Right addition", color: "rgba(220, 38, 38, 0.24)", strike: true });
    expect(coloredText(rightView)).toContainEqual({ text: "Left addition", color: "rgba(220, 38, 38, 0.24)", strike: true });
    expect(coloredText(rightView)).toContainEqual({ text: "Right addition", color: "rgba(22, 163, 74, 0.24)", strike: false });
    expect(decorateDocumentComparisonSide(left, right, "right").body).toEqual(leftView.body);
  });

  it.each(["row", "cell", "column"] as const)("preserves adjacent and boundary %s slots in both directions", (kind) => {
    const create = (ids: readonly string[]): IDocumentData => kind === "column"
      ? structuredDocument(["Intro", "columns", "After"], { columns: ids })
      : structuredDocument(["Intro", "table", "After"], { table: kind === "row" ? ids.map((id) => [id]) : [ids] });
    const before = create(["a", "c"]);
    const after = create(["First addition", "a", "Middle addition", "Adjacent addition", "c", "Last addition"]);
    const original = JSON.stringify([before, after]);
    for (const side of ["left", "right"] as const) {
      const ghost = decorateDocumentComparisonSide(before, after, side);
      const actual = decorateDocumentComparisonSide(after, before, side === "left" ? "right" : "left");
      ...
      for (const text of ["First addition", "Middle addition", "Adjacent addition", "Last addition"]) {
        expect(coloredText(ghost)).toContainEqual({ text, color: "rgba(220, 38, 38, 0.24)", strike: true });
        expect(coloredText(actual)).toContainEqual({ text, color: "rgba(22, 163, 74, 0.24)", strike: false });
      }
      if (kind === "column") {
        expect(ghost.body?.columnGroups).toEqual(after.body?.columnGroups);
      } else {
        expect(ghost.tableSource!["table-1"]!.tableColumns).toEqual(after.tableSource!["table-1"]!.tableColumns);
        expect(ghost.tableSource!["table-1"]!.tableRows.map((row) => row.tableCells.length))
  ...
  it.each(["row", "cell", "column"] as const)("mirrors a missing whole %s inside an existing container", (kind) => {
    const before = kind === "column"
      ? structuredDocument(["Intro", "columns", "After"], { columns: ["a", "c"] })
      : structuredDocument(["Intro", "table", "After"], { table: kind === "row" ? [["a"], ["c"]] : [["a", "c"]] });
    const after = kind === "column"
      ? structuredDocument(["Intro", "columns", "After"], { columns: ["a", "Added content", "c"] })
      : structuredDocument(["Intro", "table", "After"], { table: kind === "row" ? [["a"], ["Added content"], ["c"]] : [["a", "Added content", "c"]] });
    const original = JSON.stringify([before, after]);
    ...
    for (const side of ["left", "right"] as const) {
      ...
      expect(validateDocumentStructure(ghost)).toEqual([]);
      expect(validateDocumentStructure(actual)).toEqual([]);
      expect(coloredText(ghost)).toContainEqual({ text: "Added content", color: "rgba(220, 38, 38, 0.24)", strike: true });
      expect(coloredText(actual)).toContainEqual({ text: "Added content", color: "rgba(22, 163, 74, 0.24)", strike: false });
      if (kind === "column") {
        expect(ghost.body?.columnGroups?.[0]?.columns).toEqual(after.body?.columnGroups?.[0]?.columns);
      } else {
        const table = ghost.tableSource!["table-1"]!;
        expect(table.tableRows.map((row) => row.tableCells.length)).toEqual(after.tableSource!["table-1"]!.tableRows.map((row) => row.tableCells.length));
        expect(table.tableColumns).toEqual(after.tableSource!["table-1"]!.tableColumns);
        expect(table.tableRows[kind === "row" ? 1 : 0]?.tableCells[kind === "cell" ? 1 : 0]?.backgroundColor?.rgb).toBe("rgba(220, 38, 38, 0.12)");
        expect(actual.tableSource!["table-1"]!.tableRows[kind === "row" ? 1 : 0]?.tableCells[kind === "cell" ? 1 : 0]?.backgroundColor?.rgb).toBe("rgba(22, 163, 74, 0.12)");
      }
    }
    expect(JSON.stringify([before, after])).toBe(original);
  });
  ...
  it.each([
    ["table", "cell"], ["columns", "column-1"], ["quote", "block-copy"], ["nested", "cell"],
  ].flatMap(([kind, targetId]) => ["before", "after"].map((placement) => [kind!, targetId!, placement])))
  ("mirrors inserted and empty paragraphs inside an existing %s (%s, %s)", (kind, targetId, placement) => {
    const before = structuredDocument(["Intro", kind, "After"]);
    const after = structuredDocument(["Intro", kind, "After"]);
    const body = after.body!;
    const paragraphEnd = body.paragraphs!.find((paragraph) => paragraph.paragraphId === targetId)!.startIndex;
    const start = kind === "quote" ? body.blockRanges![0]!.startIndex + 1
    ...
    TextX.apply(body, [
      ...
      { t: TextXActionType.INSERT, len: text.length + 2, body: {
        dataStream: `${text}\r\r`,
        paragraphs: [{ paragraphId: "addition", startIndex: text.length }, { paragraphId: "empty-addition", startIndex: text.length + 1 }],
      } },
    ]);
    const original = JSON.stringify([before, after]);
    expect(validateDocumentStructure(after)).toEqual([]);
    for (const side of ["left", "right"] as const) {
      ...
      expect(validateDocumentStructure(ghost)).toEqual([]);
      expect(validateDocumentStructure(actual)).toEqual([]);
      for (const key of ["tables", "columnGroups", "blockRanges", "sectionBreaks", "customRanges"] as const) {
  ...
  it.each(["table", "columns", "quote", "callout", "code", "nested"] as const)(
    ...
    (kind) => {
      ...
      for (const side of ["left", "right"] as const) {
        ...
        const actual = decorateDocumentComparisonSide(present, absent, side === "left" ? "right" : "left");
        expect(ghost.body?.dataStream).toBe(present.body?.dataStream);
        expect(actual.body?.dataStream).toBe(present.body?.dataStream);
        expect(validateDocumentStructure(ghost)).toEqual([]);
        expect(validateDocumentStructure(actual)).toEqual([]);
        for (const key of ["tables", "columnGroups", "blockRanges", "customRanges", "customBlocks"] as const) {
        ...
        if (evidence >= 0) {
          for (const rendered of [ghost, actual]) {
            expect(rendered.body?.textRuns?.find((run) => run.st <= evidence && run.ed > evidence)?.ts?.bl)
              .toBe(BooleanNumber.TRUE);
          }
        }
        expect(coloredText(ghost).some((run) => run.color === "rgba(220, 38, 38, 0.24)" && run.strike)).toBe(true);
        expect(coloredText(actual).some((run) => run.color === "rgba(22, 163, 74, 0.24)" && !run.strike)).toBe(true);
        if (present.tableSource?.["table-1"] !== undefined) {
          expect(ghost.tableSource?.["table-1"]?.tableRows[0]?.tableCells[0]?.backgroundColor?.rgb)
            .toBe("rgba(220, 38, 38, 0.12)");
          expect(actual.tableSource?.["table-1"]?.tableRows[0]?.tableCells[0]?.backgroundColor?.rgb)
            .toBe("rgba(22, 163, 74, 0.12)");
        }
        expect(new Set(ghost.body?.paragraphs?.map((paragraph) => paragraph.paragraphId)).size)
  ...
  it("keeps adjacent structural and ordinary ghosts in SDK alignment order", () => {
    ...
    const after = structuredDocument(["Intro", "Before table", "table", "Between", "columns", "After"]);
    const rendered = decorateDocumentComparisonSide(before, after, "left");
    expect(rendered.body?.dataStream).toBe(after.body?.dataStream);
    expect(validateDocumentStructure(rendered)).toEqual([]);
  });
  ...
  it("does not infer missing structures from snapshots without SDK differences", () => {
    ...
    const rendered = renderDocumentComparisonSide(before, after, "left", { items: [], alignment: [] });
  ...
  it("never invents a difference when the API reports an unchanged paragraph", () => {
    ...
    const rendered = renderDocumentComparisonSide(left, right, "left", {
      items: [],
      alignment: [{
        id: "paired", stableId: "paragraph-1", kind: "equal", moved: false,
        leftIndex: 0, rightIndex: 0,
        leftNativeStableId: "paragraph-1", rightNativeStableId: "paragraph-1",
      }],
    });
  ...
  it("uses SDK character segments without splitting a supplementary Unicode character", () => {
    ...
    expect(coloredText(left)).toContainEqual({ text: "😀", color: "rgba(37, 99, 235, 0.22)", strike: false });
    expect(coloredText(right)).toContainEqual({ text: "🤝", color: "rgba(37, 99, 235, 0.22)", strike: false });
  });
...
function structuredDocument(parts: readonly string[], options?: { readonly table?: readonly (readonly string[])[]; readonly columns?: readonly string[] }): IDocumentData {
  const T = DataStreamTreeTokenType;
  const data: IDocumentData = {
    id: "doc-structures", documentStyle: {},
    body: { dataStream: "", paragraphs: [], sectionBreaks: [], tables: [], columnGroups: [], blockRanges: [], customRanges: [], customBlocks: [], textRuns: [] },
  };
  const body = data.body!;
  const token = (value: string): void => { body.dataStream += value; };
  ...
  const table = (): void => {
    ...
    (data.tableSource ??= {})["table-1"] = {
      tableId: "table-1",
      tableRows: grid.map((row) => ({ tableCells: row.map(() => ({})), trHeight: { val: { v: 24 }, hRule: TableRowHeightRule.AT_LEAST } })),
      tableColumns: Array.from({ length: Math.max(...grid.map((row) => row.length)) }, () => ({ size: { type: TableSizeType.SPECIFIED, width: { v: 260 } } })),
      align: TableAlignmentType.START, indent: { v: 0 }, textWrap: TableTextWrapType.NONE,
      ...
      size: { type: TableSizeType.SPECIFIED, width: { v: 260 * Math.max(...grid.map((row) => row.length)) } },
    };
  };
  const block = (blockType: DocumentBlockRangeType): void => {
    ...
    token("Evidence");
    (body.textRuns ??= []).push({ st: textStart, ed: body.dataStream.length, ts: { bl: BooleanNumber.TRUE } });
    const rangeEnd = body.dataStream.length;
    token(T.CUSTOM_RANGE_END);
    body.customRanges!.push({ rangeId: "link-1", rangeType: CustomRangeType.HYPERLINK, startIndex: rangeStart, endIndex: rangeEnd, properties: { url: "https://example.com/evidence" } });
    paragraph("block-copy", " for review");
    token(T.BLOCK_END);
    body.blockRanges!.push({ blockId: "block-1", blockType, startIndex, endIndex: body.dataStream.length - 1 });
  };
  for (const part of parts) {
    if (part === "table") table();
      ...
      for (const [index, id] of columns.entries()) {
        ...
        else paragraph(id, id === "column-1" ? "First column" : id === "column-2" ? "Second column" : id);
        if (part === "nested" && index === 0) table();
        token(T.COLUMN_END);
      }
      token(T.COLUMN_GROUP_END);
      body.columnGroups!.push({ columnGroupId: "columns-1", startIndex, endIndex: body.dataStream.length - 1, columns: columns.map((id) => ({ columnId: id, widthRatio: 1 })) });
