...
} from "../src/controllers/import-range-formula.controller.js";
import {
  createImportRangeFunction,
  IMPORT_RANGE_FORMULA_NAME
} from "../src/functions/import-range.function.js";
...
describe("ImportRangeFormulaController", () => {
  it("registers IMPORTRANGE as an async formula", () => {
    ...
    const registerFunctionService = {
      registerAsyncFunction
    } as IImportRangeFunctionRegistrar;

    const controller = new ImportRangeFormulaController(
...
describe("createImportRangeFunction", () => {
  ...
  it("returns #REF! for embed read-data reference errors", async () => {
    const formula = createImportRangeFunction({
  ...
  it("propagates non-embed read-data failures", async () => {
    const formula = createImportRangeFunction({
      readData: vi.fn(async () => {
  ...
  it("propagates embed read-data failures outside missing-reference codes", async () => {
    const formula = createImportRangeFunction({
      readData: vi.fn(async () => {
    ...
    await expect(formula("#unit=sales-source&type=sheet", "Sheet1!A1")).rejects.toMatchObject({
      code: EmbedErrorCode.LocalRuntimeResourceRefDataSelectorUnsupported
    });
  });
});
