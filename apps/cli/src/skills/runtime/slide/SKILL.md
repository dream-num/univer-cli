---
name: slide
description: "Create, edit, inspect, and visually verify Slide units with the Lite Interface."
---
...
Stored text boxes may contain a white fill, gray line, dark 16-point text, and a 260 × 88 default size. A bare shape may store null fill and stroke while the renderer supplies an opaque blue theme fill. Storage inspection therefore does not prove rendered color.
...
Images are local assets only. Never place a full-page reference screenshot behind editable elements; it leaves artifacts and destroys editability. For decorative noise, dots, or grids, tile a small bitmap rather than creating hundreds of tiny shapes.
...
For an exact complete order, use the reorder command (each call commits and is undoable):
...
real series to appear, and capture that rendered page.
...
- No visual surface verifies transitions: screenshots, structured inspection, and layout lint all
  ignore them. The `getTransition()` read-back is the only evidence; include it in your report when
  a task requires transitions.
...
`univer api show FSlideTable` gives the rest.
