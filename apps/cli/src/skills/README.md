...
`discovery/univer-cli` 是可按名称读取的 discovery Skill，不进入默认 list 或 `get --all`。Skill 内容必须与当前
command、CLI SDK capability 和 Univer SDK Facade 保持一致；command contract 改动时同步更新对应 Skill 和测试。
