...
`print` uses the CLI SDK `unit-print-pdf` capability and the current browser Render Page to write Sheet, Doc, Slide, or Board Units as PDF. Base Units are not printable. The command reads trunk by default; `--worktree` prints the selected Worktree without modifying it.
